...
-(void)buildItems
{
    ...
    self.backItem.onTapClick = ^(TbBaseItem *item){
        ...
        testPageView = [[MyPageView alloc] initWithFrame:rect];
        [self.pdfViewCtrl appendPageView:testPageView];
        [self.pdfViewCtrl registerGestureEventListener:(id<IGestureEventListener>)testPageView];
...
- (BOOL)onTap:(UITapGestureRecognizer *)recognizer
{
    ...
    if (width * 0.2 < point.x && point.x < width * 0.8) {
        ...
        if(testPageView)
            [((id<IGestureEventListener>)testPageView) onTap:recognizer];
