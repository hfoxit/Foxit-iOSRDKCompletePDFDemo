...
static NSString *openingFilePath = nil;
...
- (id)initWithPath:(NSString *)filePath modifiedDate:(NSDate *)lastModifiedDate isFavorite:(NSNumber *)isFavorite
{
    ...
    if(self = [super init])
    {
        ...
        {
            //set isOpen property
            if(openingFilePath != nil && [openingFilePath isEqualToString:self.path])
                //cannot use property otherwise it will trigger save to DB FileRecent table
                _isOpen = YES;
            else
                _isOpen = NO;
        }
